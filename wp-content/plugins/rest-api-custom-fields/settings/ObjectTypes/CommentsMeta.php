		...
		foreach ($this->metaItemKeys as $metaItem) {
			add_settings_field(
				...
				function ($args) use($metaItem) {
					...
					?>
					<input type="checkbox" name="integromat_api_options_comment[<?php echo esc_attr($args['label_for']); ?>]" value="1" <?php if (isset($options[$args['label_for']]) && $options[$args['label_for']] == 1) echo 'checked' ?> id="<?php echo  IMAPIE_FIELD_PREFIX  . $metaItem ?>">
					<?php
				},
				'integromat_api_comment',
				'integromat_api_section_comments',
